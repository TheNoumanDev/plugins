...
// Point at our own tvOS core pod (upstream firebase_core has no tvOS platform).
...
class AppCheckProviderWrapper: NSObject, AppCheckProvider {
  ...
  func configure(
    ...
    case "appAttestWithDeviceCheckFallback":
      ...
      // Attest, which is false on the tvOS simulator. The SDK only reports that
      // when a token is first requested, so check the same capability it checks
    ...
    case "recaptcha":
      // `RecaptchaProvider` is unavailable in the tvOS Firebase SDK (reCAPTCHA
      // App Check is not a tvOS provider — tvOS uses DeviceCheck / App Attest).
      // Keep the case so Pigeon dispatch stays intact; on tvOS leave the
      // provider unconfigured so getToken surfaces an explicit error instead of
      // failing silently.
      #if os(iOS)
