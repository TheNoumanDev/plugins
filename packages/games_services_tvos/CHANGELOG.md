...
- On 4.x, note that `signIn()` resolves with a success *message* rather
  than `null` — see "Using this with `games_services` 4.x" in the README.
  Failure throws on both versions, so a completed call is the reliable
  success signal.
